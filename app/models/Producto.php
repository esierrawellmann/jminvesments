...
class Producto extends database {
  ...
  {
    $this->conectar();
    $query = $this->consulta("select p.id_producto,p.nombre,t.nombre  as 'tipo_producto',precio_compra,precio_venta,cantidad from producto p inner join tipo_producto t on p.id_tipo_producto = t.id_tipo_producto order by p.id_producto");
    $this->disconnect();
  ...
  function newProducto($producto){
    ...
    $queryObject = $this->consulta("select p.id_producto,p.nombre,t.nombre  as 'tipo_producto',precio_compra,precio_venta,cantidad from producto p inner join tipo_producto t on p.id_tipo_producto = t.id_tipo_producto order by p.id_producto DESC LIMIT 1; ");
    $this->disconnect();
